...
def ConvModel(n_classes, sample_rate=44100, duration=.2,
...
def load_audio(file_path, label):
    ...
    audio, sample_rate = tf.audio.decode_wav(audio, desired_channels=1, desired_samples=8820)
    audio = tf.squeeze(audio)
    return audio, label


def prepare_for_training(ds, shuffle_buffer_size=256, batch_size=32):
...
def prepare_for_testing(ds, shuffle_buffer_size=256, batch_size=32):
    # Randomly shuffle (file_path, label) dataset
    ds = ds.shuffle(buffer_size=shuffle_buffer_size)
    # Load and decode audio from file paths
    ds = ds.map(load_audio, num_parallel_calls=AUTOTUNE)
...
def main():
    ...
    print(results)
    #toprow is initial 
    #toprow2 is with validation
    #toprow3 is with 8820 as the amount of samples instead of 44100 (.2 sec instead of 1 sec)
    model.save("./toprow3.h5", include_optimizer=True)
